...
// }
  ...
  const handleCreateProjectSuccess = async () => {
    enqueueSnackbar('Se ha registrado su pedido, sera atendido en la brevedad posible.', {
      variant: 'success',
      anchorOrigin: {
        vertical: 'top',
        horizontal: 'right'
  ...
  const enviarRequest = async () => {
    try {
      ...
      const response = await certifyAxios.post('/pedido_bar/new', reqObj)
  ...
  const llamarProductosPorSala = async() =>{
    ...
    responseActive.data.result.forEach(element => {
      if(element.marca==="PILSEN" || element.marca==="PAN CON POLLO"){
      x.push({
  ...
  useEffect(()=>{
    let x = JSON.parse(localStorage.getItem("pedido_bar"));
    if(x===null) navigate('/bar_order/read');
    setCliente(x);
    console.log(x);
    ws.onopen = () => {
    ...
    ws.onmessage = (event) => {
      ...
      // console.log(data);
      if(data.body.codigo==="3002") {
        navigate('/bar_order/read');
      }
      else
      {
        // setTarjeta('');
  ...
  return (
    <>
      <Grid container
            ...
            direction="column"
            alignItems="left"
            justifyContent="center" sx={{mb:2}} style={{display: 'inline-block'}}>   
             
          <Typography variant="h1" component="h1" style={{display: 'inline-block'}}>
            Realizar un pedido
          </Typography>   
          {/* <Typography variant="h2" component="h2" style={{display: 'inline-block', marginLeft:'10%'}}>
            Cliente: 
          </Typography>    */}
        ...
        boxShadow: "none"}}> */}
            {/* <Grid xs={4}/> */}
            <ImageList gap={2} cols={2} rowHeight={280} sx={{'&::-webkit-scrollbar': {display: 'none'}}}>
