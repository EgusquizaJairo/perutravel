...
const ResultsActive = () => {
  ...
  const enviarRequest = async () => {
    try {
      ...
      await certifyAxios.post('/pago_manual/update', request);
      ws.send(
        JSON.stringify({
          sender: "Empleado1",
          body: "1002",
  ...
  const handleCreateProjectSuccess = () => {
    enqueueSnackbar('Se ha atendido el pago manual exitosamente', {
      variant: 'success',
      anchorOrigin: {
        vertical: 'top',
        horizontal: 'right'
  ...
  const getPagosManuales = useCallback(async () => {
    try {
      // reqObj.suspended = "0"; // Active
      // Active Associates
      console.log("Quiero obtener los pagos manuales");
      let reqObj = {};
      const responseActive = await certifyAxios.post('/pago_manual/select/', reqObj);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      const data = JSON.parse(event.data);
      if(data.body==="1001") getPagosManuales();
    };
    getPagosManuales();
    // return () => {
                ...
                <TableBody>
                  {paginatedAssociated.map((associated) => {
                    return (
                      <>
                      <TableRow >
                        <TableCell align='center'>Id. Pago Manual</TableCell>
                        ...
                        <TableCell align='center'>Nombre del cliente</TableCell>
                         ...
                         <TableRow >
                          ...
                          <TableCell align='center'>
                           ...
                           onClick={()=>{
                            handleClickOpen();
                            setIdPagoManual(associated.id_pago_manual);
                            }}>
                             Atender
                           </Button>
                          </TableCell>
                          ...
                          open={open}
                            ...
                            >
                              <DialogTitle id="alert-dialog-title">
                                ¿Desea atendar el pago manual?
                              </DialogTitle>
                              <DialogContent>
                                <DialogContentText id="alert-dialog-description">
                                  Se atendará el pago manual.
                                </DialogContentText>
                              </DialogContent>
                              <DialogActions>
