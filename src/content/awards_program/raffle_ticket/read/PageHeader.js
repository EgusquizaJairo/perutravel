...
// }
  ...
  const buscarCliente = async (nroTarjeta) =>{
    ...
    let responseActive = await certifyAxios.post('/person/client/card', reqObj);
    // console.log("sadsadsad");
    // console.log(responseActive);
    // let pedidoBar = {"name":nroTarjeta};
    // console.log(nroTarjeta);
    let nuevo = responseActive.data.result[0];
    nuevo.tarjeta = nroTarjeta;
    localStorage.setItem("raffle_client",JSON.stringify(nuevo));
    navigate('/raffle_ticket/base');
  }
  ...
  return (
    <>
      <Grid container
            ...
            direction="column"
            alignItems="center"
            justifyContent="center" sx={{mb:2}} style={{marginTop:'100px'}}>    
          <Typography variant="h3" component="h3" style={{width:'300px',alignContent:'center',justifyContent:'center'}}>
            Por favor ingrese una tarjeta para continuar
          </Typography>  
          <br/> 
          <img alt="xd" src={imagen}/>
      </Grid>
